package com.lambdapractice;

import java.util.Arrays;

interface MyComparator{
	public int salComparator(Employee emp1,Employee emp2);
}

public class EmpSalSortLambda {

	
	public static void main(String args[]) {
		
		Employee e1=new Employee(1,"pooji",55000);
		Employee e2=new Employee(2,"lakshmi",12000);
		Employee e3=new Employee(3,"hari",1000000);
		Employee e4=new Employee(4,"lally",25000);
		Employee e5=new Employee(5,"shreya",10000);
		
		Employee[] ea= {e1,e2,e3,e4,e5};
		
		MyComparator comparator=(emp1,emp2)->Double.compare(emp1.getSal(),emp2.getSal());
		Arrays.sort(ea,(emp3,emp4)->comparator.salComparator(e1,e2));
		
		for(Employee e:ea)
			System.out.println(e);
		
	}

}
