...
public class App 
{
    public static void main( String[] args )
    {
      ...
      System.out.println("-----------using string array and streams-----------------");
      //stream of strings
      String[] strs= {"apple","guava","watermelon","mango","papaya","grapes","kiwi","pomegranate","sapodilla","green apple","black grapes","cherrys"};
      Arrays.stream(strs).filter(name->name.length()>5).forEach(System.out::println);
      System.out.println("----------startsWith s---------------");
      Arrays.stream(strs).filter(name->name.length()>5)
      ...
      Employee e1=new Employee("pooji",123,30000,"java developer");
      Employee e2=new Employee("Thanu",124,25000,"python developer");
      Employee e3=new Employee("jaya",125,45000,"manager");
      Employee e4=new Employee("priya",126,20000,"manager");
      Employee e5=new Employee("sohel",127,35000,"Tester");
     Employee[] err1= {e1,e2,e3,e4,e5};
      
      ArrayList<Employee> err=new ArrayList<>(Arrays.asList(err1));
      
      System.out.println("-----------Employees having salary greater than 27000---------------");
      err.stream().filter(emp->emp.getSal()>27000).forEach(System.out::println);
      
      System.out.println("-----employees whos designation is manager---------");
      err.stream().filter(emp->emp.getDesignation()
    		  .equals("manager"))
      			.sorted((emp1,emp2)->emp1.getEname().compareTo(emp2.getEname())).forEach(System.out::println);
      
      System.out.println("----------employee names--------------");
      err.stream().map(e->e.getEname()).forEach(System.out::println);;
//      System.out.println("------------sorting employees------------");
//      err.stream().sorted().forEach(System.out::println);
